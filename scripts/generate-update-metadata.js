...
function generateUpdateMetadata(version, platform) {
    ...
    if (platform === 'darwin') {
        ...
        const SHA512 = crypto.createHash('sha512').update(FILE_BUFFER).digest('base64');
...
`;
        ...
        const RELEASES_PATH = path.join(RELEASE_DIR, 'squirrel.windows', 'x64', 'RELEASES');
        ...
        const NUPKG_PATH = path.join(RELEASE_DIR, 'squirrel.windows', 'x64', NUPKG_FILE);
...
`;
        ...
        const OUTPUT_PATH = path.join(RELEASE_DIR, 'squirrel.windows', 'x64', 'latest.yml');
        fs.writeFileSync(OUTPUT_PATH, YAML_CONTENT);
        console.log(`Generated: ${OUTPUT_PATH}`);
    }
}
...
if (!VERSION || !PLATFORM) {
    console.error('Usage: node generate-update-metadata.js <version> <platform>');
